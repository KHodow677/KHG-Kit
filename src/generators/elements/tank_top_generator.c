...
void generate_tank_top(tank_top *tt, tank_body *tb, float x, float y) {
  generate_follower(&tt->follower_info, &tb->physics_info, 102.0f, 209.0f, 1.0f, phy_v(x, y), 0.0f, phy_v(0.0f, 55.5f));
  generate_renderer(&tt->renderer_info, tt->follower_info.body, TANK_TOP);
