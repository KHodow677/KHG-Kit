...
void generate_turret_top(turret_top *tt, turret_base *tb, float x, float y, float angle) {
  ...
  generate_renderer(tt->comp_renderer, tb->comp_renderer, tt->comp_physics, TURRET_TOP, 3);
  ...
  generate_targeter(tt->comp_targeter, tb->comp_physics, tt->comp_physics, 640.0f);
