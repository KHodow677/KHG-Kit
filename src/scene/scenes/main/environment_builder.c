...
void build_environment_element(const int tex_id, const float x, const float y, const float angle, const bool is_animated, const int render_layer, const float parallax_value) {
  ...
  comp_physics_constructor_info comp_physics_ci = { PHYSICS_BOX, 0.0f, 0.0f, 1.0f, phy_vector2_new(x, y), angle, false, false, false };
  const comp_physics *cp = sys_physics_add(entity, &comp_physics_ci);
  comp_renderer_constructor_info comp_renderer_ci = { cp->body, cp->shape, tex_id, NO_RIG_ID, NO_AREA_ID, render_layer, parallax_value, false, false };
