...
void build_ovr_tile(const char *tile_id, const phy_vector2 pos) {
  ...
  element_comp_render comp_renderer_ci = { .tex_id_loc = NAMESPACE_TASKING()->get_texture_id("EMPTY_TEXTURE"), .ovr_tile = (ovr_tile_info){ NAMESPACE_TASKING()->get_tile_id(tile_id), pos }, .render_layer = 0, .parallax_value = 1.0f, .flipped = false };
