...
static ecs_ret sys_zone_update(ecs_ecs *ecs, ecs_id *entities, const unsigned int entity_count, const ecs_dt dt, void *udata) {
  if (dt == 0.0f) {
    return 0;
  }
  for (unsigned int id = 0; id < entity_count; id++) {
    comp_zone *info = ecs_get(ECS, entities[id], ZONE_COMPONENT_SIGNATURE);
    const float body_x = phy_rigid_body_get_position(info->body).x;
