...
static int compare_ovr_tile_elements(const void *a, const void *b) {
  ...
  float arg1 = ovr_tile_rendering_pos(e1->parent_tile->pos, e1->pos, NAMESPACE_LOADING()->get_tex_def((char *)e1->element_tex_id).height * OVR_TILE_SCALE, 0.0f).y;
  float arg2 = ovr_tile_rendering_pos(e2->parent_tile->pos, e2->pos, NAMESPACE_LOADING()->get_tex_def((char *)e2->element_tex_id).height * OVR_TILE_SCALE, 0.0f).y;
...
void render_ovr_tile_item(char *tex_id, const phy_vector2 coords, const phy_vector2 offset) {
  gfx_texture tex_ref = NAMESPACE_LOADING()->get_tex_def(tex_id);
...
void render_ovr_tile_element_item(const ovr_tile_element *element) {
  gfx_texture tex_ref = NAMESPACE_LOADING()->get_tex_def((char *)element->element_tex_id);
  tex_ref.width *= OVR_TILE_SCALE;
  tex_ref.height *= OVR_TILE_SCALE;
...
void render_ovr_tile(const ovr_tile_info *tile, unsigned int *layer, const float dt) {
  ...
  switch (*layer) {
    case GROUND_LAYER:
      render_ovr_tile_item((char *)tile_ref.ground_tex_id, tile->pos, phy_vector2_new(OVR_TILE_RAW_POSITION, OVR_TILE_RAW_POSITION));
      break;
    case ELEMENT_LAYER:
      ...
      break;
    case BORDER_LAYER:
      render_ovr_tile_item((char *)tile_ref.border_tex_id, tile->pos, phy_vector2_new(OVR_TILE_RAW_POSITION, OVR_TILE_RAW_POSITION));
      break;
    default:
      break;
  }
