...
void generate_tex_defs(const char *filename) {
  ...
  NAMESPACE_TASKING_INTERNAL.TEXTURE_ASSET_THREAD.max = utl_vector_size(TEXTURE_NAMES);
  NAMESPACE_TASKING_INTERNAL.TEXTURE_RAW_THREAD.max = utl_vector_size(TEXTURE_NAMES);
  NAMESPACE_TASKING_INTERNAL.TEXTURE_THREAD.max = utl_vector_size(TEXTURE_NAMES);
}
...
int emplace_tex_defs(void *arg) {
  ...
  while (thread->progress < num_names) {
    ...
    NAMESPACE_TASKING_INTERNAL.TEXTURE_THREAD.progress++;
