...
void dbm_index_init(dbm_index *index) {
  dbm_btree_init(&index->search);
  dbm_btree_init(&index->freelist);
  index->fd = NULL;
}

dbm_db_state dbm_index_open(dbm_index *index, const char *path, long long row_count, long long freelist_count) {
  ...
  if (!is_new_file) {
    ...
    for (long long i = 0; i < row_count; i++) {
      ...
      dbm_btree_insert(&index->search, node.key, node.value);
    }
    for (long long i = 0; i < freelist_count; i++) {
      fread(&node.key, sizeof(node.key), 1, fd);
      fread(&node.value, sizeof(node.value), 1, fd);
      dbm_btree_insert(&index->freelist, node.key, node.value);
    }
  }
  return DBM_OK;
}
...
void dbm_index_close(dbm_index *index) {
  ...
  fflush(index->fd);
  fclose(index->fd);
  dbm_btree_destroy(&index->search);
  dbm_btree_destroy(&index->freelist);
}
