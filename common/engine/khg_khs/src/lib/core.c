...
static unsigned long TAPE[] = { 59243892, 2014914089654949231, 120301499583420, 23, 3230239239, 120302102103, 904924490212, 10412  };
...
static khs_val khs_num_to_str(khs_float num) {
  ...
  for (size_t i = 0; i < n_ddigits; i++) {
    dec_part *= 10;
    char digit = khs_int_as_digit(((unsigned long) dec_part) % 10);
    khs_write_char_at(&s, i, digit);
  }
  return res;
}
...
static unsigned long khs_random_from(unsigned long from) {
  for(size_t i = 0; i < KHS_TAPE_LEN; i++) {
    TAPE[i] *= TAPE[i] + (from+1);
  }
  size_t tape_head = 0;
  unsigned long res = from;
  for (unsigned long i = 1; i != 0; i = i << 1) {
    char bit = from & 1;
    from = from >> 1;
    if (bit) {
      tape_head += res + 1;
...
static khs_val khs_random_callback(const khs_val *args, khs_size n_args) {
  (void)n_args, (void)args;
  static unsigned long seed = (unsigned long)&khs_random_callback;
...
static khs_val khs_floor_callback(const khs_val *args, khs_size n_args) {
  ...
  return KHS_NUMBER((unsigned long)khs_as_num(args[0]));
}
...
static khs_val khs_ceil_callback(const khs_val *args, khs_size n_args) {
  ...
  khs_float res = (unsigned long)khs_as_num(args[0]);
  res += 1;
  return KHS_NUMBER(res);
}
