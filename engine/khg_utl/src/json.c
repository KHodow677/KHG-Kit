...
static utl_json_element* parse_array(utl_json_parser_state* state);
static utl_json_element* parse_string(utl_json_parser_state* state);
static utl_json_element* parse_number(utl_json_parser_state* state);
static utl_json_element* parse_null(utl_json_parser_state* state);
static utl_json_element* parse_boolean(utl_json_parser_state* state);
static utl_json_element* parser_internal(utl_json_parser_state* state);
static utl_json_element* parse_object(utl_json_parser_state* state);
static void json_serialize_internal(const utl_json_element* element, utl_string *str);
static bool json_find_in_object(const utl_json_element* object, utl_json_predicate predicate, void* user_data, utl_json_element** found_element);
static bool json_find_in_array(const utl_json_element* array, utl_json_predicate predicate, void* user_data, utl_json_element** found_element);
static void json_format_internal(const utl_json_element* element, utl_string* str, int indent);

static void next_token(utl_json_parser_state* state) {
...
static utl_json_element* parse_array(utl_json_parser_state* state) {
  ...
  utl_json_element* array_element = json_create(UTL_JSON_ARRAY);
  if (!array_element) {
  ...
  while (state->current_token.type != UTL_JSON_TOKEN_ARRAY_END) {
    ...
    utl_json_element* element = parser_internal(state);
    if (!element) {
...
static utl_json_element* parse_string(utl_json_parser_state* state) {
  ...
  state->position++;
  utl_json_element* element = json_create(UTL_JSON_STRING);
  if (!element) {
...
static utl_json_element* parse_number(utl_json_parser_state* state) {
  ...
  free(number_str);
  utl_json_element* element = json_create(UTL_JSON_NUMBER);
  if (!element) {
...
static utl_json_element* parse_null(utl_json_parser_state* state) {
  ...
  state->position += 4;
  utl_json_element* element = json_create(UTL_JSON_NULL);
  if (!element) {
...
static utl_json_element* parse_boolean(utl_json_parser_state* state) {
  ...
  utl_json_element* element = json_create(UTL_JSON_BOOL);
  if (!element) {
...
static utl_json_element* parse_object(utl_json_parser_state* state) {
  ...
  utl_json_element* object_element = json_create(UTL_JSON_OBJECT);
  if (!object_element) {
  ...
  while (state->current_token.type != UTL_JSON_TOKEN_OBJECT_END) {
    ...
    next_token(state);
    utl_json_element* value = parser_internal(state);
    if (!value) {
      free(key);
      json_deallocate(object_element);
      return NULL;
...
static void serialize_array(const utl_json_element* element, utl_string* str) {
  utl_string_append(str, "[");
  for (size_t i = 0; i < utl_vector_size(element->value.array_val); ++i) {
    json_serialize_internal(*(utl_json_element**)utl_vector_at(element->value.array_val, i), str);
    if (i < utl_vector_size(element->value.array_val) - 1) {
      utl_string_append(str, ", ");
    }
  }
...
static void serialize_object(const utl_json_element* element, utl_string* str) {
  ...
  while (it.node != end.node) {
    ...
    utl_string_append(str, ": ");
    json_serialize_internal((utl_json_element*)it.node->value, str);
    first = false;
    utl_map_iterator_increment(&it);
  }
  utl_string_append(str, "}");
...
static void format_array(const utl_json_element* element, utl_string* str, int indent) {
  ...
  for (size_t i = 0; i < utl_vector_size(element->value.array_val); ++i) {
    ...
    json_format_internal(*(utl_json_element**)utl_vector_at(element->value.array_val, i), str, indent + 2);
    if (i < utl_vector_size(element->value.array_val) - 1) {
      utl_string_append(str, ",");
    }
    utl_string_append(str, "\n");
...
static void format_object(const utl_json_element* element, utl_string* str, int indent) {
  ...
  while (it.node != end.node) {
    ...
    utl_string_append(str, ": ");
    json_format_internal((utl_json_element*)it.node->value, str, indent + 2);
    first = false;
    utl_map_iterator_increment(&it);
  }
  if (!first) {
...
utl_json_element * json_read_from_file(const char* filename) {
  ...
  buffer[size] = '\0';
  utl_json_element* json_element = json_parse(buffer);
  if (!json_element) {
...
utl_json_element* json_deep_copy(const utl_json_element *element) {
  ...
  utl_json_element *copy = json_create(element->type);
  if (!copy) {
    utl_error_func("Memory allocation failed", utl_user_defined_data);
    return NULL;
  }
  ...
  switch (element->type) {
    ...
    case UTL_JSON_ARRAY:
      for (size_t i = 0; i < utl_vector_size(element->value.array_val); ++i) {
        utl_json_element *item = json_deep_copy(*(utl_json_element **)utl_vector_at(element->value.array_val, i));
        if (!item) {
          json_deallocate(copy);
    ...
    case UTL_JSON_OBJECT:
      ...
      while (it.node != end.node) {
        ...
        utl_json_element *val_copy = json_deep_copy((utl_json_element *)it.node->value);
        if (!val_copy) {
          free(key);
          json_deallocate(copy);
...
utl_json_element* json_merge(const utl_json_element *element1, const utl_json_element *element2) {
  ...
  utl_json_element* merged = json_create(UTL_JSON_OBJECT);
  if (!merged) {
  ...
  while (it.node != end.node) {
    ...
    utl_json_element* value_copy = json_deep_copy((utl_json_element*)it.node->value);
    if (!value_copy) {
  ...
  switch (type) {
    case UTL_JSON_STRING:
      if (element->type == UTL_JSON_NUMBER) {
        utl_json_element *stringElement = json_create(UTL_JSON_STRING);
        if (!stringElement) {
    ...
    case UTL_JSON_BOOL:
      ...
      return NULL;
    case UTL_JSON_ARRAY: {
      utl_json_element *array = json_create(UTL_JSON_ARRAY);
      if (!array) {
        utl_error_func("Memory allocation failed for array", utl_user_defined_data);  
        return NULL;
      }
      switch (element->type) {
        case UTL_JSON_STRING: {
          ...
          while (i < len) {
            ...
            utl_json_element *charElement = json_create(UTL_JSON_STRING);
      ...
      break;
    case UTL_JSON_OBJECT: {
      utl_json_element *object = json_create(UTL_JSON_OBJECT);
      if (!object) {
...
utl_json_element* json_map(const utl_json_element* array, utl_json_map_function map_func, void* user_data) {
  ...
  utl_json_element* resultArray = json_create(UTL_JSON_ARRAY);
  if (!resultArray) {
...
utl_json_element* json_filter(const utl_json_element *array, utl_json_predicate predicate, void *user_data) {
  ...
  for (size_t i = 0; i < utl_vector_size(array->value.array_val); ++i) {
    ...
    if (predicate(currentElement, user_data)) {
      utl_json_element* elementCopy = json_deep_copy(currentElement);
      if (!elementCopy) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (bracketPos) {
      ...
      *endBracket = '\0';
      ...
      if (json_type_of_element(currentElement) == UTL_JSON_ARRAY) {
        currentElement = json_get_element(currentElement, indexStr);
        if (!currentElement) {
          utl_error_func("JSON Element not found", utl_user_defined_data);
          break;
        }
