...
gfx_shader shader_prg_create(const char *vert_src, const char *frag_src) {
...
void shader_set_mat(gfx_shader prg, const char *name, mat4 mat) {
...
gfx_text_props text_render_simple(vec2s pos, const char *text, gfx_font font, gfx_color font_color, bool no_render) {
  return gfx_text_render(pos, text, font, font_color, -1, (vec2s){-1, -1}, no_render, false, -1, -1);
}

gfx_text_props text_render_simple_wide(vec2s pos, const wchar_t *text, gfx_font font, gfx_color font_color, bool no_render) {
  return gfx_text_render_wchar(pos, text, font, font_color, -1, (vec2s){-1, -1}, no_render, false, -1, -1);
}

gfx_clickable_item_state button(const char *file, int32_t line, vec2s pos, vec2s size, gfx_element_props props, gfx_color color, float border_width,  bool click_color, bool hover_color) {
  return button_ex(file, line, pos, size, props, color, border_width, click_color, hover_color, (vec2s){ -1, -1 }); 
}

gfx_clickable_item_state button_ex(const char *file, int32_t line, vec2s pos, vec2s size, gfx_element_props props, gfx_color color, float border_width, bool click_color, bool hover_color, vec2s hitbox_override) {
...
gfx_clickable_item_state div_container(vec2s pos, vec2s size, gfx_element_props props, gfx_color color, float border_width, bool click_color, bool hover_color) {
  if(item_should_cull((gfx_aabb){ .pos = pos, .size = size })) {
    return gfx_clickable_idle;
  }
...
bool item_should_cull(gfx_aabb item) {
  bool intersect = true;
...
void draw_scrollbar_on(gfx_div *div) {
  gfx_next_line();
...
void input_field(gfx_input_field *input, gfx_input_field_type type, const char *file, int32_t line) {
  if(!input->buf) {
    return;
  }
  if(!input->init) {
  ...
  gfx_font font = get_current_font();
  ...
  gfx_clickable_item_state inputfield = button(file, line, input_aabb.pos, input_aabb.size, props, props.color, props.border_width, false, false);
  if(gfx_mouse_button_went_down(GLFW_MOUSE_BUTTON_LEFT) && input->selected && inputfield == gfx_clickable_idle) {
    ...
    gfx_text_props selected_props = gfx_text_render((vec2s){
      state.pos_ptr.x + props.padding, 
      state.pos_ptr.y + props.padding
    }, input->buf, font, gfx_no_color, wrap_point, (vec2s){gfx_get_mouse_x(), gfx_get_mouse_y()}, true, false, -1, -1);
    input->cursor_index = selected_props.rendered_count;
  }
  if(input->selected) {
...
gfx_clickable_item_state button_element_loc(void *text, const char *file, int32_t line, bool wide) {
  ...
  if(wide) {
    text_props = text_render_simple_wide(state.pos_ptr, (const wchar_t *)text, font, gfx_no_color, true);
  }
  else {
    text_props = text_render_simple(state.pos_ptr, (const char*)text, font, gfx_no_color, true);
  }
  ...
  gfx_clickable_item_state ret = button(file, line, state.pos_ptr, (vec2s){ text_props.width + padding * 2, text_props.height + padding * 2 }, props, color, props.border_width, true, true);
  if(wide) {
...
gfx_clickable_item_state button_fixed_element_loc(void *text, float width, float height, const char *file, int32_t line, bool wide) {
  gfx_element_props props = get_props_for(state.theme.button_props);
  float padding = props.padding;
  float margin_left = props.margin_left, margin_right = props.margin_right,
  margin_top = props.margin_top, margin_bottom = props.margin_bottom;
  ...
  state.pos_ptr.x += margin_left;
  state.pos_ptr.y += margin_top; 
  gfx_clickable_item_state ret = button(file, line, state.pos_ptr, (vec2s){ render_width + padding * 2.0f, render_height + padding * 2.0f }, props, color, props.border_width, false, true);
...
gfx_clickable_item_state checkbox_element_loc(void *text, bool *val, gfx_color tick_color, gfx_color tex_color, const char *file, int32_t line, bool wide) {
  gfx_font font = get_current_font();
  ...
  gfx_clickable_item_state checkbox = button(file, line, state.pos_ptr, (vec2s){ checkbox_size + props.padding * 2.0f, checkbox_size + props.padding * 2.0f }, props, checkbox_color, props.border_width, false, false);
  if(wide) {
...
void dropdown_menu_item_loc(void **items, void *placeholder, uint32_t item_count, float width, float height, int32_t *selected_index, bool *opened, const char *file, int32_t line, bool wide) {
  ...
  gfx_clickable_item_state dropdown_button = button(file, line, state.pos_ptr, (vec2s){ (float)width + padding * 2.0f, (float)text_props.height + padding * 2.0f },  props, props.color, props.border_width, false, true);
  if(wide) {
...
int32_t menu_item_list_item_loc(void **items, uint32_t item_count, int32_t selected_index, gfx_menu_item_callback per_cb, bool vertical, const char *file, int32_t line, bool wide) {
  gfx_element_props props = get_props_for(state.theme.button_props);
  float padding = props.padding;
  float margin_left = props.margin_left, margin_right = props.margin_right,
  margin_top = props.margin_top, margin_bottom = props.margin_bottom; 
