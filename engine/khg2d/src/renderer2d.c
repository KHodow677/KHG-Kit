...
void render_text(renderer_2d *r2d, vec2 position, const char *text, const font f, const vec4 color, const float size, const float spacing, const float line_space, short show_in_center, const vec4 shadow_color, const vec4 light_color) {
  if (f.texture.id == 0) {
    error_func("Missing font", user_defined_data);
    return;
  } 
...
void render_text_wrapped(renderer_2d *r2d, const char *text, font f, vec4 text_pos, vec4 color, float base_size, float spacing, float line_spacing, short show_in_center, vec4 shadow_color, vec4 light_color) {
  char *new_text;
...
void flush_fbo(renderer_2d *r2d, framebuffer fb, short should_clear) {
  if (fb.fbo == 0) {
