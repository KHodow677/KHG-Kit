...
size_t map_max_size(const Map *map) {
  if (map == NULL) {
    error_func(stderr, "Error: Null pointer provided for map in map_max_size.\n");
    return 0;
  }
...
MapIterator map_crbegin(const Map *map) {
  MapIterator iterator = {0};
  if (map == NULL) {
    error_func(stderr, "Error: Null pointer provided for map in map_crbegin.\n");
    return iterator;
  }
  if (map != NULL && map->root != NULL) {
    MapNode *current = map->root;
...
MapIterator map_crend(const Map *map) {
  if (map == NULL) {
    error_func(stderr, "Error: Null pointer provided for map in map_crend.\n");
    return (MapIterator){0};
  }
  return (MapIterator){0};
}
