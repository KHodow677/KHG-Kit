...
mat4 mat4CreateIdentity(void) {
  mat4 mat4 = mat4CreateSameValue(0.0f);
  mat4.values[0] = 1.0f;
  mat4.values[5] = 1.0f;
  mat4.values[10] = 1.0f;
  mat4.values[15] = 1.0f;
...
mat4 mat4Scale(mat4 *transform, vec3 *scale) {
  mat4 transform_scale = mat4CreateIdentity();
  ...
  transform_scale = mat4Multiply(transform, &transform_scale);
  return transform_scale;
}

mat4 mat4Translate(mat4 *transform, vec3 *translate) {
  mat4 transform_translate = mat4CreateIdentity();
  ...
  transform_translate = mat4Multiply(transform, &transform_translate);
...
mat4 mat4Rotate(mat4 *transform, vec3 *axis, float angle) {
  ...
  transform_rotate = mat4Multiply(transform, &transform_rotate);
...
mat4 mat4Orthographic(float left, float right, float bottom, float top, float near_plane, float far_plane) {
  mat4 orthographic = mat4CreateSameValue(0.0f);
...
mat4 mat4Perspective(float fov, float aspect_ratio, float near_plane, float far_plane) {
  ...
  perspective.values[10] = -((far_plane + near_plane) / (far_plane - near_plane));
  perspective.values[11] = -1.0f;
  perspective.values[14] = -((2.0f * far_plane * near_plane) / (far_plane - near_plane));
  return perspective;
}
