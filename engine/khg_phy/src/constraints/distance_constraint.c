...
void phy_distance_constraint_presolve(phy_space *space, phy_constraint *cons, float dt, float inv_dt) {
  phy_distance_constraint *dist_cons = (phy_distance_constraint *)cons->def;
  ...
  dist_cons->mass = 1.0 / phy_calc_mass_k(dist_cons->normal, dist_cons->xanchor_a, dist_cons->xanchor_b, invmass_a, invmass_b, invinertia_a, invinertia_b);
...
void phy_distance_constraint_warmstart(phy_space *space, phy_constraint *cons) {
  ...
  if (space->settings.warmstarting) {
    ...
    if (a) {
      phy_rigid_body_apply_impulse(cons->a, phy_vector2_neg(impulse), dist_cons->xanchor_a);
    }
    if (b) {
      phy_rigid_body_apply_impulse(cons->b, impulse, dist_cons->xanchor_b);
    }
  }
  else {
    dist_cons->impulse = 0.0;
...
void phy_distance_constraint_solve(phy_constraint *cons) {
  ...
  phy_vector2 rv = phy_calc_relative_velocity(linear_velocity_a, angular_velocity_a, dist_cons->xanchor_a, linear_velocity_b, angular_velocity_b, dist_cons->xanchor_b);
  ...
  if (a) {
    phy_rigid_body_apply_impulse(a, phy_vector2_neg(impulse), dist_cons->xanchor_a);
  }
  if (b) {
    phy_rigid_body_apply_impulse(b, impulse, dist_cons->xanchor_b);
  }
}
