...
void phy_hinge_constraint_presolve(
    ...
    if (phy_vector2_len2(delta) == 0.0) hinge_cons->normal = phy_vector2_zero;
    else hinge_cons->normal = phy_vector2_normalize(delta);
    ...
    hinge_cons->mass = 1.0 / phy_calc_mass_k(
        hinge_cons->normal,
...
void phy_hinge_constraint_solve(phy_constraint *cons, float inv_dt) {
    ...
    if (a) phy_rigid_body_apply_impulse(a, phy_vector2_neg(impulse), hinge_cons->xanchor_a);
    if (b) phy_rigid_body_apply_impulse(b, impulse, hinge_cons->xanchor_b);
}
