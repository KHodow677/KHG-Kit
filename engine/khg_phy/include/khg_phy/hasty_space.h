...
CP_EXPORT phy_space *cpHastySpaceNew(void);
CP_EXPORT void cpHastySpaceFree(phy_space *space);
...
CP_EXPORT void cpHastySpaceSetThreads(phy_space *space, unsigned long threads);

/// Returns the number of threads the solver is using to run.
CP_EXPORT unsigned long cpHastySpaceGetThreads(phy_space *space);

/// When stepping a hasty space, you must use this function.
CP_EXPORT void cpHastySpaceStep(phy_space *space, float dt);
