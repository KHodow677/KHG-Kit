...
#define KHS_POP_ARG(to, err_msg) { if(n_args > 0) { n_args--; to = *args; args++; } else { return BERYL_ERR("Unexpected end of arguments; " err_msg); } }
#define KHS_EXPECT_TYPE(val, type, err_msg) { if(BERYL_TYPEOF(val) != type) { beryl_blame_arg(val); return BERYL_ERR(err_msg); } }

#define KHS_MATH_OP(name, start_val, start_index, op)\
static i_val name(const i_val *args, khs_size n_args) {\
