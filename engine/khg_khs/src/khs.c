...
#define STATIC_STACK_SIZE 256
#define GLOBALS_LOOKUP_TABLE_SIZE 256
#define EXPR_ARG_STACK_SIZE 128
...
typedef struct scope_namespace {
...
static scope_namespace current_namespace = { NULL, NULL };
static stack_entry globals_lookup[GLOBALS_LOOKUP_TABLE_SIZE];

static i_val arg_stack[EXPR_ARG_STACK_SIZE];
static i_val *arg_stack_top = arg_stack, *arg_stack_end = arg_stack + EXPR_ARG_STACK_SIZE;
...
static void blame_token(lex_state *lex, lex_token tok) {
	push_stack_trace((stack_trace_entry){ 0, lex->src, lex->end, tok.src, tok.len });
}

static void blame_range(lex_state *lex, const char *start, const char *end) {
...
static bool namespace_overlap(scope_namespace a, scope_namespace b) {
...
i_val beryl_new_object(beryl_object_class *obj_class) {
	...
	beryl_object *obj = beryl_alloc(obj_class->obj_size);
	if (obj == NULL) {
		return BERYL_NULL;
  }
	obj->ref_c = 1;
...
static i_val parse_eval_fn_assign(lex_state *lex, bool eval, lex_token var_tok,  lex_token fn_assign) {
...
static i_val parse_eval_term(lex_state *lex, bool eval) {
	lex_token tok = lex_pop(lex);
	switch (tok.type) {
		case TOK_NUMBER:
		...
		case TOK_OPEN_BRACKET: {
			...
			lex_token end_bracket = lex_pop(lex);
      ...
      else if(lex_accept(lex, TOK_FN_ASSIGN, &fn_assign)) {
      ...
      if (current_namespace.start != NULL) {
				scope_namespace namespace = global? (scope_namespace){ NULL, NULL } : current_namespace;
				...
				if (var->name != NULL) {
					beryl_release(assign_val);
					blame_token(lex, var_sym);
					return BERYL_ERR("Redeclaration of global variable");
				}
				var->name = var_name;
		...
		case TOK_FN: {
			while (!lex_accept(lex, TOK_DO, NULL)) {
				lex_token arg = lex_pop(lex);
        if (arg.type == TOK_VARARGS) {
					lex_token varargs_ident = lex_pop(lex);
...
static i_val parse_eval_subexpr(lex_state *lex, bool eval) {
...
static bool continue_expr(lex_state *lex) {
...
i_val parse_eval_all_exprs(lex_state *lex, bool eval, unsigned char until_tok, lex_token *end_tok) {
...
i_val call_internal_fn(i_val fn, const i_val *args, i_size n_args) {
  ...
  if (is_variadic) {
	...
	scope_namespace prev_namespace = current_namespace;
	current_namespace = (scope_namespace){ fn.val.fn, fn.val.fn + fn.len };
	...
	if (is_variadic) {
		lex_token variadic_tok = lex_pop(&lex);
		lex_token varargs_name = lex_pop(&lex);
		assert(variadic_tok.type == TOK_VARARGS);
		assert(varargs_name.type == TOK_SYM || varargs_name.type == TOK_OP);
		if (get_local(varargs_name.content.sym.str, varargs_name.content.sym.len) != NULL) {
...
i_val beryl_call(i_val fn, const i_val *args, size_t n_args, bool borrow) {
	...
	switch (fn.type) {
		case TYPE_EXT_FN: {
			beryl_external_fn *ext_fn = fn.val.ext_fn;
      if (ext_fn->arity < 0) {
...
i_val beryl_eval(const char *src, size_t src_len, enum beryl_err_action err) {
	...
	stack_entry *prev_scope = enter_scope();
	scope_namespace prev_namespace = current_namespace;
	current_namespace = (scope_namespace){ NULL, NULL };
	i_val res = parse_eval_all_exprs(&lex, true, TOK_EOF, NULL);
	leave_scope(prev_scope);
	current_namespace = prev_namespace;
	if (BERYL_TYPEOF(res) == TYPE_ERR) {
